...
public class AuthController {
    ...
    public ResponseEntity<ApiResponse<TokenRefreshResponse>> refreshToken(@Valid @RequestBody TokenRefreshRequest request) {
        ...
        return refreshTokenService.findByToken(requestRefreshToken)
                ...
                .map(com.waad.tba.modules.auth.entity.RefreshToken::getUser)
                .map(user -> {
                    // Generate new Access Token using User entity directly
                    String token = tokenProvider.generateToken(user);
                    
                    return ResponseEntity.ok(ApiResponse.success(new TokenRefreshResponse(token, requestRefreshToken)));
                })
                .orElseThrow(() -> new com.waad.tba.common.exception.TokenRefreshException(requestRefreshToken, "Refresh token is not in database!"));
    }
}

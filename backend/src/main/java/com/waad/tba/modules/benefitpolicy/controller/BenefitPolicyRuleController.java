...
public class BenefitPolicyRuleController {
    ...
    public ResponseEntity<ApiResponse<Page<BenefitPolicyRuleResponseDto>>> findAllPaged(
        ...
        Page<BenefitPolicyRuleResponseDto> result = ruleService.findByPolicy(policyId, label, encounterType, pageable);
        return ResponseEntity.ok(ApiResponse.success("Rules retrieved", result));
    }

    @GetMapping("/rules/deleted")
    @PreAuthorize("hasAuthority('benefit_policies.view') or hasRole('SUPER_ADMIN')")
    @Operation(summary = "List all deleted rules for a policy (paginated)")
    public ResponseEntity<ApiResponse<Page<BenefitPolicyRuleResponseDto>>> findDeletedPaged(
            @PathVariable Long policyId,
            @RequestParam(required = false) String label,
            @RequestParam(required = false) com.waad.tba.modules.visit.entity.VisitType encounterType,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "20") int size) {
        Pageable pageable = PageRequest.of(page, size);
        Page<BenefitPolicyRuleResponseDto> result = ruleService.findDeletedByPolicy(policyId, label, encounterType, pageable);
        return ResponseEntity.ok(ApiResponse.success("Deleted rules retrieved", result));
    }
    ...
    @PreAuthorize("hasAuthority('benefit_policies.update') or hasRole('SUPER_ADMIN')")
    @Operation(summary = "Restore a soft-deleted rule")

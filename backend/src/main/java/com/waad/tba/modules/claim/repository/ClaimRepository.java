...
public interface ClaimRepository extends JpaRepository<Claim, Long> {
       ...
       /**
        * Count claims by status filtered by employer organization.
        */
       @Query("SELECT COUNT(c) FROM Claim c WHERE c.active = true " +
                     "AND c.member.employerOrganization.id = :employerOrgId " +
                     "AND c.status = :status")
       long countByStatusAndEmployerOrganizationId(
                     @Param("status") com.waad.tba.modules.claim.entity.ClaimStatus status,
       ...
       default long countApprovedClaimsByMemberAndServiceInPeriod(
              ...
              return countPendingAndApprovedClaimsByMemberAndServiceInPeriod(
                            memberId,
                            String.valueOf(serviceCategoryId),
                            startDate,
                            endDate);
       }

       /**
        * Count claim usage by member and service code in a date range.
        *
        * Uses claim lines service_code (snapshot) and counts distinct claims to avoid
        * double counting when a claim has multiple lines for the same service.
        */
       @Query("SELECT COUNT(DISTINCT c.id) FROM Claim c " +
                     "JOIN c.lines cl " +
                     "WHERE c.active = true " +
                     ...
                     "AND c.serviceDate >= :startDate " +
                     "AND c.serviceDate <= :endDate")
       ...
       /**
        * Settled amounts by provider (all employers).
        * Returns: [providerId, settledNetAmount]
        */
       @Query("SELECT c.providerId, COALESCE(SUM(COALESCE(c.netProviderAmount, c.approvedAmount)), 0) " +
                     "FROM Claim c " +
                     "WHERE c.active = true " +
                     "AND c.status = com.waad.tba.modules.claim.entity.ClaimStatus.SETTLED " +
                     "GROUP BY c.providerId")
       List<Object[]> getSettledAmountsByProvider();

       /**
        * Settled amounts by provider filtered by employer.
        * Returns: [providerId, settledNetAmount]
        */
       @Query("SELECT c.providerId, COALESCE(SUM(COALESCE(c.netProviderAmount, c.approvedAmount)), 0) " +
                     "FROM Claim c " +
                     ...
                     "AND c.status = com.waad.tba.modules.claim.entity.ClaimStatus.SETTLED " +
                     "GROUP BY c.providerId")
       List<Object[]> getSettledAmountsByProviderAndEmployer(@Param("employerOrgId") Long employerOrgId);
        ...
        * NATIVE QUERY used to avoid Hibernate join pollution and ensure robust
       ...
       @Query("DELETE FROM Claim c WHERE c.member.id = :memberId")

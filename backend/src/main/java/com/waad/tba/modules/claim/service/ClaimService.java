...
public class ClaimService {
    ...
    @Transactional(readOnly = true)
    public Page<ClaimViewDto> listClaims(Long employerId, int page, int size, String sortBy, String sortDir,
            String search) {
        log.debug(
                "📋 Listing claims with pagination. employerId={}, page={}, size={}, sortBy={}, sortDir={}, search={}",
    ...
    public ClaimViewDto submitClaim(Long id) {
        ...
        LocalDate expectedCompletionDate = businessDaysCalculator.calculateExpectedCompletionDate(submissionDate,
                slaDays);
    ...
    /**
     * Get pending claims inbox statistics.
     */
    @Transactional(readOnly = true)
    public com.waad.tba.modules.claim.dto.InboxStatsDto getPendingClaimsStats() {
        User currentUser = authorizationService.getCurrentUser();
        ...
        long total = claimRepository.countInboxClaims(pendingStatuses, providerId);
        long submitted = claimRepository.countInboxClaimsByStatus(ClaimStatus.SUBMITTED, providerId);
        long underReview = claimRepository.countInboxClaimsByStatus(ClaimStatus.UNDER_REVIEW, providerId);

        java.time.LocalDateTime startOfDay = LocalDate.now().atStartOfDay();
        long todayNew = claimRepository.countInboxClaimsToday(startOfDay, pendingStatuses, providerId);

        BigDecimal totalRequested = claimRepository.sumInboxClaimsRequestedAmount(pendingStatuses, providerId);
        BigDecimal avgAmount = BigDecimal.ZERO;

        if (total > 0 && totalRequested != null) {
            avgAmount = totalRequested.divide(new BigDecimal(total), 2, java.math.RoundingMode.HALF_UP);
        }

        return com.waad.tba.modules.claim.dto.InboxStatsDto.builder()
                .total(total)
                .submitted(submitted)
    ...
    public ClaimViewDto returnForInfo(Long id, ClaimReturnForInfoDto dto) {
        ...
        if (claim.getStatus() != ClaimStatus.UNDER_REVIEW) {
            throw new BusinessRuleException(
                    ...
                    "claim.return.invalid",
                    new Object[] { claim.getStatus() });
        }
        ...
        if (dto.getReason() == null || dto.getReason().trim().isEmpty()) {
            throw new BusinessRuleException(
                    com.waad.tba.common.error.ErrorCode.BUSINESS_RULE_VIOLATION,
                    "claim.return.reason.required",
                    null);
        }

        // Set reviewer comment with the reason
        Locale locale = LocaleContextHolder.getLocale();
        String commentPrefix = messageSource.getMessage("claim.return.comment.prefix", null, locale);
        String fullComment = commentPrefix + dto.getReason();

        if (dto.getRequiredDocuments() != null && !dto.getRequiredDocuments().trim().isEmpty()) {
            String docsPrefix = messageSource.getMessage("claim.return.documents.prefix", null, locale);
            fullComment += "\n\n" + docsPrefix + dto.getRequiredDocuments();
        }

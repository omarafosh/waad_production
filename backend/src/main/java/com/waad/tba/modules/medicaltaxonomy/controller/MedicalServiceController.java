...
public class MedicalServiceController {
    ...
    public ResponseEntity<ApiResponse<Page<MedicalServiceResponseDto>>> findAll(
            ...
            @Parameter(description = "Filter by Master status: true=master only, false=non-master only, null=all") @RequestParam(required = false) Boolean isMaster,
            @Parameter(description = "Search term (code/name)") @RequestParam(required = false) String searchTerm) {

        log.info("[MEDICAL-SERVICES] GET /api/medical-services - page={}, size={}, active={}, isMaster={}, search={}", page, size, active, isMaster, searchTerm);

        Sort sort = Sort.by(Sort.Direction.fromString(sortDir), sortBy);
        Pageable pageable = PageRequest.of(page, size, sort);
        Page<MedicalServiceResponseDto> result = serviceService.findAll(pageable, active, isMaster, searchTerm);
    ...
    @PreAuthorize("hasRole('SUPER_ADMIN') or hasRole('INSURANCE_ADMIN')")
    @Operation(summary = "Smart Reclassify Service", description = "Reclassify service with coverage strategy (INHERIT, KEEP_OVERRIDE, CUSTOM)")
    public ResponseEntity<ApiResponse<MedicalServiceResponseDto>> reclassify(
            @PathVariable Long id,
            @Valid @RequestBody com.waad.tba.modules.medicaltaxonomy.dto.ReclassifyRequestDto dto) {

        log.info("[MEDICAL-SERVICES] POST /api/medical-services/{}/reclassify - strategy={}", id, dto.getStrategy());

        MedicalServiceResponseDto result = serviceService.reclassify(id, dto);

        return ResponseEntity.ok(ApiResponse.success("Service reclassified successfully", result));
    }

    @GetMapping("/{id}/reclassify/impact")
    @PreAuthorize("hasRole('SUPER_ADMIN') or hasRole('INSURANCE_ADMIN')")
    @Operation(summary = "Get Reclassification Impact", description = "Analyze impact of changing service category")
    public ResponseEntity<ApiResponse<java.util.Map<String, Object>>> getReclassifyImpact(
            @PathVariable Long id,
            @RequestParam Long newCategoryId) {

        log.info("[MEDICAL-SERVICES] GET /api/medical-services/{}/reclassify/impact - newCategoryId={}", id, newCategoryId);

        var result = serviceService.getReclassificationImpact(id, newCategoryId);

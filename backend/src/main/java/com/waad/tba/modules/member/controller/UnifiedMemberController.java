...
public class UnifiedMemberController {
    ...
    @ApiResponses(value = {
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Members retrieved successfully", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ApiResponse.class)))
    })
    ...
    public ResponseEntity<ApiResponse<Page<MemberViewDto>>> searchMembers(
        ...
        log.info("Searching Members with criteria: {}, page={}, size={}", criteria, page, size);

        Pageable pageable = PageRequest.of(page, size);
        Page<MemberViewDto> results = unifiedMemberService.searchMembersAdvanced(criteria, pageable);

        return ResponseEntity.ok(ApiResponse.success(results));
    }

    // ==================== CONSOLIDATED UNIFIED OPERATIONS ====================

    @GetMapping("/unified-search")
    @Operation(summary = "Unified member search (Auto-detection)", description = "Search by Card, Name (fuzzy), or Barcode.")
    public ResponseEntity<ApiResponse<List<MemberSearchDto>>> search(@RequestParam String query) {
        log.info("Unified search request: {}", query);
        List<MemberSearchDto> results = memberSearchService.search(query.trim());
        return ResponseEntity.ok(ApiResponse.success(results));
    }

    @GetMapping("/{id}/details")
    @Operation(summary = "Get member details by ID (Search result context)")
    public ResponseEntity<ApiResponse<MemberSearchDto>> getMemberDetails(@PathVariable Long id) {
    ...
    @Operation(summary = "Deterministic eligibility check", description = "Auto-detects Card vs Barcode.")
    public ResponseEntity<ApiResponse<EligibilityResultDto>> checkEligibilityUnified(@RequestParam String query) {
        log.info("Unified eligibility check: {}", query);
        EligibilityResultDto result = memberSearchService.checkEligibility(query);
        return ResponseEntity.ok(ApiResponse.success(result));
    }

    @GetMapping("/{memberId}/remaining-limit")
    @PreAuthorize("hasAnyRole('SUPER_ADMIN', 'INSURANCE_ADMIN', 'EMPLOYER_ADMIN', 'PROVIDER')")
    @Operation(summary = "Get Member Remaining Limit (Typed)")
    public ResponseEntity<ApiResponse<MemberRemainingLimitDto>> getRemainingLimit(@PathVariable Long memberId) {
        log.info("Retrieving remaining limit for member: {}", memberId);
        MemberFinancialSummaryDto summary = financialSummaryService.getFinancialSummary(memberId);

        MemberRemainingLimitDto dto = MemberRemainingLimitDto.builder()
                .memberId(memberId)
    ...
    @Operation(summary = "Download Beneficiaries PDF Report", description = "Generates and downloads a PDF report of beneficiaries matching the specified filters.")
    ...
    public ResponseEntity<ApiResponse<MemberViewDto>> promoteToActive(@PathVariable Long id,
    ...
    public ResponseEntity<List<com.waad.tba.modules.member.entity.MemberWorkflowHistory>> getWorkflowHistory(
            @PathVariable Long id) {
    ...
    public ResponseEntity<ApiResponse<com.waad.tba.modules.member.entity.MemberDocument>> uploadDocument(
        ...
        com.waad.tba.modules.member.entity.MemberDocument doc = memberDocumentService.uploadDocument(id, file, type,
                currentUser);
     ...
     * Improved: Handles missing files gracefully by returning a placeholder or
     * empty content

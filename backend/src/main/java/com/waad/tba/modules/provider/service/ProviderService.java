...
public class ProviderService {
    ...
    public ProviderViewDto updateProvider(Long id, ProviderUpdateDto dto) {
        ...
        if (dto.getAllowedPayers() != null) {
            ...
            // Remove unselected (orphanRemoval sets active=false or deletes. Here we
            // delete)
    ...
    public List<AllowedEmployerDto> getAllowedEmployers(Long providerId) {
        ...
        // 3. Add Contract Model Employers (The "Contracted" List) checking for
        // duplicates
    ...
    public void syncUserWithProvider(User user) {
        ...
        providerRepository.findById(user.getProviderId())
                .ifPresent(provider -> syncVisibilityToUsers(provider, java.util.Collections.singletonList(user)));
    }
    ...
    public void syncProviderFromUser(User user) {
        ...
        // Sync back to ALL other users linked to this provider (excluding the current
        // user to avoid loops)

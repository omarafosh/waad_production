...
public class SettlementBatchService {
    ...
    public BatchSummaryDTO createBatch(CreateBatchRequest request, Long userId) {
        ...
        // 1. Verify provider has an account (create if not exists handled by
        // accountService usually, but here checking existence)
    ...
    private String generateBatchNumber() {
        ...
        // Locking strategy for number generation is better handled by DB sequence or
    ...
    public void addClaimsToBatch(Long batchId, List<Long> claimIds, Long userId) {
        ...
        for (Claim claim : claims) {
            // Assuming Claim has providerId. Need to check Claim entity structure in
            // context or assumed standard
        ...
        for (Claim claim : claims) {
            ...
            // Or we rely on the implementation that "Available Claims" query filters out
            // those in batch_items
        ...
        // Actually best to re-fetch items or use repo aggregation, but entity
        // Recalculate works if items loaded
    ...
    public void removeClaimsFromBatch(Long batchId, List<Long> claimIds, Long userId) {
        ...
        // Not implementing claim status revert here, relying on batchItem check for
        // availability
    ...
    public void payBatch(Long batchId, String paymentReference, PaymentMethod method, Long userId) {
        ...
        // accountService.processBatchPayment(batch.getProviderAccountId(),
        // paymentAmount, batchId, batch.getBatchNumber(), userId);
        ...
        List<Long> claimIds = batchItemRepository.findBySettlementBatchId(batchId).stream()
                .map(SettlementBatchItem::getClaimId)
                .collect(Collectors.toList());

        if (claimIds.isEmpty()) {
            throw new IllegalStateException("Cannot pay an empty batch");
        }

        List<Claim> claims = claimRepository.findAllById(claimIds);
        if (claims.size() != claimIds.size()) {
            throw new IllegalStateException("Some claims in batch are missing");
        }

        LocalDateTime settledAt = LocalDateTime.now();
        String settlementNotes = "Settled via batch " + batch.getBatchNumber();
        for (Claim claim : claims) {
            if (claim.getStatus() != ClaimStatus.APPROVED) {
                throw new IllegalStateException(
                        "Cannot settle claim " + claim.getId() + " in status " + claim.getStatus());
            }
    ...
    public void cancelBatch(Long batchId, String reason, Long userId) {
        ...
        // If items existed, they are effectively "released" because the batch is
        // cancelled
        // If we want to physically delete items or keep them for audit of cancelled
        // batch?
        // Usually we keep them but finding available claims must exclude cancelled
        // batches?
        // Or we delete them?
        // The entity comments said "CANCELLED: Void, claims returned to APPROVED".
        // To return to approved, we just need to make sure they are not linked to an
        // active batch.
        // If we simply delete items, they become available again.
        // If we keep items, we need to ensure "Available Claims" query filters out
        // items only in non-cancelled batches.
    ...
    public BatchSummaryDTO getBatchSummary(Long batchId) {
        ...
        Provider provider = accountRepository.findById(batch.getProviderAccountId())
                .flatMap(account -> providerRepository.findById(account.getProviderId()))
                .orElse(null);
    ...
    public List<AvailableClaimDTO> getAvailableClaims(Long providerId) {
        return claimRepository.findByProviderId(providerId).stream()
                .filter(claim -> claim.getStatus() == ClaimStatus.APPROVED)
                .filter(claim -> !batchItemRepository.existsByClaimId(claim.getId()))
                .map(claim -> {
                    AvailableClaimDTO dto = AvailableClaimDTO.fromClaim(claim);
                    dto.setMemberName(claim.getMember() != null ? claim.getMember().getFullName() : null);
                    dto.setProviderId(claim.getProviderId());

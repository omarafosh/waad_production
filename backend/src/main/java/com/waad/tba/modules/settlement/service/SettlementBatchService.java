...
public class SettlementBatchService {
    ...
    public void payBatch(Long batchId, String paymentReference, PaymentMethod method, Long userId) {
        ...
        for (Long cid : claimIds) {
            claimRepository.findById(cid).ifPresent(c -> {
                ...
                // Leaving detailed Claim status update implementation to ClaimService
                // integration phase or assuming helper exists.
            });
        }
    ...
    public BatchSummaryDTO getBatchSummary(Long batchId) {
        ...
        Provider provider = providerRepository.findById(
                accountRepository.findById(batch.getProviderAccountId()).get().getProviderId()).orElse(null);
    ...
    /**
     * Get claims available for batching for a provider
     */
    public List<AvailableClaimDTO> getAvailableClaims(Long providerId) {
        // Find claims that are APPROVED and not in any batch
        List<Claim> availableClaims = claimRepository.findAvailableForSettlement(providerId);

        return availableClaims.stream()

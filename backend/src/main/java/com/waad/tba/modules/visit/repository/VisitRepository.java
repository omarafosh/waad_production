...
public interface VisitRepository extends JpaRepository<Visit, Long> {
       ...
       Page<Visit> searchPagedByEmployerId(@Param("q") String q, @Param("employerId") Long employerId,
                     Pageable pageable);
       ...
       // PROVIDER filtering - visits by provider ID (providerId is Long field, not
       // relation)
       ...
       @Query("SELECT v FROM Visit v " +
                     ...
                     "LOWER(v.diagnosis) LIKE LOWER(CONCAT('%', :q, '%')) OR " +
                     "LOWER(m.fullName) LIKE LOWER(CONCAT('%', :q, '%')))")
       Page<Visit> searchPagedByProviderId(@Param("q") String q, @Param("providerId") Long providerId,
                     Pageable pageable);
       ...
       @Query(value = "SELECT v.* FROM visits v " +
                     ...
                     "ORDER BY v.visit_date DESC, v.id DESC", countQuery = "SELECT COUNT(*) FROM visits v " +
                                   "LEFT JOIN members m ON m.id = v.member_id " +
                                   "WHERE v.active = true " +
                                   "AND (CAST(:providerId AS BIGINT) IS NULL OR v.provider_id = CAST(:providerId AS BIGINT)) "
                                   +
                                   ...
                                   "AND (CAST(:status AS VARCHAR) IS NULL OR v.status = CAST(:status AS VARCHAR)) " +
                                   "AND (CAST(:fromDate AS DATE) IS NULL OR v.visit_date >= CAST(:fromDate AS DATE)) " +
                                   "AND (CAST(:toDate AS DATE) IS NULL OR v.visit_date <= CAST(:toDate AS DATE))", nativeQuery = true)

...
-- Unified Raw Services (Depends on ent_medical_services in V03)
...
-- Junction Table: Provider Service Mappings (Depends on providers in V06 and ent_medical_services in V03)
CREATE TABLE IF NOT EXISTS provider_service_mappings (
    ...
    CONSTRAINT fk_psm_master_service FOREIGN KEY (master_service_id) REFERENCES ent_medical_services(id),
...
-- Mapping Audit & Governance (Depends on ent_provider_raw_services and ent_medical_services)

          ...
          <Box sx={{ mb: 3 }}>
            ...
            <Paper variant="outlined" sx={{ p: 2, mb: 2 }}>
              <Box display="flex" gap={2} flexWrap="wrap">
                <Chip
                  label={`إجمالي الصفوف: ${result.summary?.total || 0}`}
                  color="default"
                  size="small"
                />
                <Chip
                  label={`تم الإنشاء: ${result.summary?.inserted || 0}`}
                  color="success"
                  size="small"
                />
                <Chip
                  label={`تم التحديث: ${result.summary?.updated || 0}`}
                  color="info"
                  size="small"
                />
                {(result.summary?.failed || 0) > 0 && (
                  <Chip
                    label={`فشل: ${result.summary?.failed || 0}`}
                    color="warning"
                    size="small"
                  />
                )}

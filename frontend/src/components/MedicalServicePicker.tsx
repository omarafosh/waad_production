...
const MedicalServicePicker = ({
  ...
  const filteredServices = useMemo(() => {
    ...
    return services.filter(s => 
      (s.category || s.categoryName) === categoryFilter
    );
        ...
        <Stack direction="row" alignItems="center" spacing={2} sx={{ width: '100%', py: 0.5 }}>
          ...
          <Box sx={{ flex: 1, minWidth: 0 }}>
            ...
            <Typography variant="body2" fontWeight="medium" sx={{ mt: 0.5 }}>
              {option.nameAr || option.name || option.serviceName}
            </Typography>
          </Box>
          ...
          {isSelected && (
            <Chip label="مضافة" size="small" color="info" />
          )}
        </Stack>
      </Box>
    );
  ...
  const filterOptions = (options, state) => {
    ...
    return options.filter(opt => {
      ...
      const nameAr = (opt.nameAr || opt.serviceNameArabic || '').toLowerCase();
    ...
    <Paper elevation={0} sx={{ p: 2, bgcolor: 'grey.50', borderRadius: 2, mb: 2 }}>
      <Stack spacing={2}>
        ...
        <Autocomplete
          ...
          getOptionLabel={(option) => option ? `${option.code || option.serviceCode} - ${option.nameAr || option.name}` : ''}
          value={searchValue}

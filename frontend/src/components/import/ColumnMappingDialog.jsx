...
const ColumnMappingDialog = ({ open, onClose, onConfirm, detectionData }) => {
  ...
  const SYSTEM_FIELDS = {
    fullName: { label: 'الاسم الكامل (عربي)', mandatory: true },
    employer: { label: 'جهة العمل', mandatory: true },
    fullNameEnglish: { label: 'الاسم (إنجليزي)', mandatory: false },
      ...
      <DialogContent dividers>
        ...
        <TableContainer component={Paper}>
          <Table size="small">
            <TableHead>
              <TableRow>
                <TableCell><strong>الحقل</strong></TableCell>
                <TableCell><strong>عمود Excel</strong></TableCell>
                <TableCell><strong>الثقة</strong></TableCell>
                <TableCell><strong>معاينة البيانات</strong></TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
        ...
        <Box mt={2} display="flex" gap={2}>
          ...
          <Chip 
            icon={<InfoIcon />}
            label={`${Object.keys(SYSTEM_FIELDS).length} حقل متاح`}
            color="info"
            variant="outlined"
          />
        </Box>

...
const MemberChronicConditionsTab = ({ memberId, readOnly = false }) => {
  ...
  const handleUpdateCoverageStatus = async () => {
    ...
    try {
      await chronicConditionsService.updateCoverageStatus(
        statusDialog.condition.id,
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      <Box>
                  ...
                  <React.Fragment key={condition.id}>
                    <TableRow hover>
                      <TableCell>
                        <IconButton
                          size="small"
                          onClick={() => setExpandedRow(expandedRow === condition.id ? null : condition.id)}
                        >
                      ...
                      <TableCell>
                        <Chip
                          ...
                          size="small"
                          onClick={!readOnly ? () => setStatusDialog({
                            open: true,
                            condition,
                      ...
                      <TableCell>
                        {condition.annualLimit 
                          ? `${Number(condition.annualLimit).toLocaleString()} ر.س`
                          : '-'
                        }
                      </TableCell>
                      <TableCell>
                        {condition.annualLimit ? (
                          <Box>
                            <Typography variant="caption">
                              {Number(condition.usedAmount || 0).toLocaleString()} ر.س
                            </Typography>
                            <LinearProgress
                              variant="determinate"
                            ...
                            <>
                              ...
                              {condition.documentationPath && !condition.documentationVerified && (
                                <Tooltip title="التحقق من المستندات">
                                  <IconButton 
                                    size="small" 
                                    color="warning"
                                    onClick={() => handleVerifyDocumentation(condition)}
                                  >
                              ...
                              <Tooltip title="حذف">
                                <IconButton 
                                  size="small" 
                                  color="error"
                                  onClick={() => setDeleteDialog({ open: true, condition })}
                                >
                                ...
                                <Card variant="outlined">
                                  <CardContent>
                                    ...
                                    <Grid container spacing={1}>
                                      ...
                                      <Grid item xs={6}>
                                        <Typography variant="caption" color="text.secondary">فترة الانتظار:</Typography>
                                        <Typography variant="body2">
                                      ...
                                      {condition.waitingPeriodEndDate && (
                                        <Grid item xs={12}>
                                          <Typography variant="caption" color="text.secondary">تنتهي في:</Typography>
                                      ...
                                      {condition.coverageReason && (
                                        <Grid item xs={12}>
                                          <Typography variant="caption" color="text.secondary">سبب حالة التغطية:</Typography>
                              ...
                              <Grid item xs={12} md={6}>
                                <Card variant="outlined">
                                  <CardContent>
                                    ...
                                    <Grid container spacing={1}>
                                      <Grid item xs={6}>
                                        <Typography variant="caption" color="text.secondary">الطبيب:</Typography>
                                        <Typography variant="body2">{condition.diagnosingPhysician || '-'}</Typography>
                                      </Grid>
                                      <Grid item xs={6}>
                                        <Typography variant="caption" color="text.secondary">المنشأة:</Typography>
                                        <Typography variant="body2">{condition.diagnosingFacility || '-'}</Typography>
                                      </Grid>
                                      <Grid item xs={12}>
                                        <Typography variant="caption" color="text.secondary">تاريخ الإفصاح:</Typography>
                                    ...
                                    <CardContent>
                                      ...
                                      <Grid container spacing={2}>
                                        {condition.currentMedications && (
                                          <Grid item xs={12} md={6}>
                                            <Typography variant="caption" color="text.secondary">الأدوية الحالية:</Typography>
                                        ...
                                        {condition.treatmentPlan && (
                                          <Grid item xs={12} md={6}>
                                            <Typography variant="caption" color="text.secondary">خطة العلاج:</Typography>
                                            <Typography variant="body2" sx={{ whiteSpace: 'pre-line' }}>
                                              {condition.treatmentPlan}
                                            </Typography>
                                          </Grid>
        ...
        >
          <DialogTitle>
            {editingCondition ? 'تعديل المرض المزمن' : 'إضافة مرض مزمن جديد'}
          </DialogTitle>
        ...
        >
          ...
          <DialogActions>
            <Button onClick={() => setStatusDialog({ open: false, condition: null, newStatus: '', reason: '' })}>
              إلغاء
            </Button>
            <Button 
              variant="contained" 
              onClick={handleUpdateCoverageStatus}

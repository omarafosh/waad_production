...
import { lookupMedicalServices, getMedicalServiceById } from 'services/api/medical-services.service';
    ...
    const syncValue = async () => {
      ...
      if (typeof value === 'number' || typeof value === 'string') {
        ...
        if (found) {
          ...
          try {
            const data = await getMedicalServiceById(id);
            if (data) {
              setSelectedService(data);
            }
          } catch (error) {
            console.error('Failed to fetch service details by ID:', error);
          }
        }
      }
    };

 ...
 */
export const getDomainForUrl = (url: string): string | null => {
  if (!url) return null;
...
export interface PermissionAwareApiHook {
  canAccess: (url: string) => boolean;
  get: <T = any>(url: string, config?: AxiosRequestConfig) => Promise<AxiosResponse<T> | { data: null; skipped: boolean }>;
  post: <T = any>(url: string, data?: any, config?: AxiosRequestConfig) => Promise<AxiosResponse<T> | { data: null; skipped: boolean }>;
  put: <T = any>(url: string, data?: any, config?: AxiosRequestConfig) => Promise<AxiosResponse<T> | { data: null; skipped: boolean }>;
  delete: <T = any>(url: string, config?: AxiosRequestConfig) => Promise<AxiosResponse<T> | { data: null; skipped: boolean }>;
}
 ...
 * }}
 */
export const usePermissionAwareApi = (): PermissionAwareApiHook => {
   ...
   * @returns {boolean}
   */
  const canAccess = useCallback((url: string): boolean => {
   ...
   */
  const get = useCallback(async <T = any>(url: string, config: AxiosRequestConfig = {}): Promise<AxiosResponse<T> | { data: null; skipped: boolean }> => {
    if (!canAccess(url)) {
   ...
   * Returns null if user doesn't have permission
   */
  const post = useCallback(async <T = any>(url: string, data?: any, config: AxiosRequestConfig = {}): Promise<AxiosResponse<T> | { data: null; skipped: boolean }> => {
    if (!canAccess(url)) {
   ...
   * Returns null if user doesn't have permission
   */
  const put = useCallback(async <T = any>(url: string, data?: any, config: AxiosRequestConfig = {}): Promise<AxiosResponse<T> | { data: null; skipped: boolean }> => {
    if (!canAccess(url)) {
   ...
   * Returns null if user doesn't have permission
   */
  const del = useCallback(async <T = any>(url: string, config: AxiosRequestConfig = {}): Promise<AxiosResponse<T> | { data: null; skipped: boolean }> => {
    if (!canAccess(url)) {
...
export interface PermissionAwareDataResult {
 ...
 */
export const usePermissionAwareData = (domain: string, fetchFn?: () => Promise<any>, deps: any[] = []): PermissionAwareDataResult => {

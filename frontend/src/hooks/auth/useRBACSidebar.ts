...
import { NavItemType } from '../ui/useMenuCollapse';
...
export interface RBACSidebarHook {
  sidebarGroups: NavItemType[];
 ...
 */
const useRBACSidebar = (): RBACSidebarHook => {
  ...
  const sidebarItems = useMemo(() => {
    ...
    const flatten = (nodes: NavItemType[]) => {
      nodes.forEach(node => {
        if (node.type === 'item') {
          items.push({
            id: node.id,

 ...
 */
export const ALL_CLAIM_STATUSES: ClaimStatusType[] = Object.values(CLAIM_STATUS);

/**
 * Arabic labels for claim statuses
 */
export const CLAIM_STATUS_LABELS: Record<ClaimStatusType, string> = {
 ...
 */
export const DEFAULT_FILTERS: ClaimsReportFilters = {
  statuses: [], // Empty = all statuses
  memberSearch: '' // Text search on member name
};

export interface ClaimsReportOptions {
...
export interface ClaimsReportPagination {
  page: number;
  size: number;
...
export interface ClaimsReportHook {
  claims: MappedClaim[];
  allClaims: MappedClaim[];
  totalCount: number;
  totalFetched: number;
  loading: boolean;
  error: string | null;
  isEmpty: boolean;
  pagination: ClaimsReportPagination;
 ...
 *
 * @param {ClaimsReportOptions} options
 * @returns {ClaimsReportHook} Claims data, loading states, error, and utilities
 *
 * Architecture: Employer → Member → Claim
 * Data Source: GET /api/claims?employerId={id}&providerId={id}
 */
export const useClaimsReport = ({ employerId, providerId, filters = DEFAULT_FILTERS }: ClaimsReportOptions = {}): ClaimsReportHook => {
  const [claims, setClaims] = useState<MappedClaim[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  const [pagination, setPagination] = useState<ClaimsReportPagination>({
    page: 0,
    size: 100,
    totalElements: 0,
    totalPages: 0
  ...
  const fetchClaims = useCallback(async () => {
    ...
    try {
      ...
      const mappedClaims: MappedClaim[] = claimsList.map((claim: any) => ({
        id: claim.id,

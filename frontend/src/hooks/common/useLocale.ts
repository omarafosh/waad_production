...
const useLocale = (): LocaleHook => {
  ...
  const locale = useMemo(() => {
    return i18n && (locales as any)[i18n] ? i18n : defaultLocale;
  }, [i18n]);

  // Get current translations object
  const translations = useMemo(() => {
    return (locales as any)[locale] || (locales as any)[defaultLocale];
  }, [locale]);
  ...
  const t = useCallback(
    (key: string, params: Record<string, any> = {}): string => {
      ...
      if (translation === undefined && locale !== 'ar') {
        translation = getNestedValue((locales as any).ar, key);
      }

      // Fallback to English if not in Arabic either
      if (translation === undefined && locale !== 'en') {
        translation = getNestedValue((locales as any).en, key);
      }

...
import { useEmployersList } from './useEmployers';

export interface EmployerScopeHook {
  user: any;
  userRole: string;
  isAdminRole: boolean;
  isEmployerLocked: boolean;
  canSelectEmployer: boolean;
  effectiveEmployerId: number | string | null;
  employers: any[];
  employersLoading: boolean;
  currentEmployerName: string;
  userEmployerId: number | string | null;
}
 ...
 */
export const useEmployerScope = (selectedEmployerId: number | string | null = null): EmployerScopeHook => {
  const { user } = useAuth();
  ...
  const currentEmployerName = useMemo(() => {
    ...
    const employer = employers.find((e: any) => e.id === effectiveEmployerId);

...
import { filterMenuByPermissions, MenuItem, User } from 'config/rbac.config';
 ...
 * @param {User | null} user - User object with permissions
 * @returns {MenuItem[]} Filtered menu items
 */
export const filterMenuByRoles = (menuItems: MenuItem[], userRoles: string[] = [], user: User | null = null): MenuItem[] => {
  // If user object lacks permissions but roles exist, construct a minimal user for the filter
  const filteringUser: User = user || { roles: userRoles, permissions: [] };

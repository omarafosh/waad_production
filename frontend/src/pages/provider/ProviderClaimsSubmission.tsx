...
import axiosClient from 'utils/axios'; // Keep for member limit if needed, or remove if unused locally
...
export default function ProviderClaimsSubmission() {
  ...
  const fetchMedicalCategories = async () => {
    ...
    try {
      const categories = await providersService.getMedicalCategories();
  ...
  const fetchVisitDetails = async () => {
    ...
    try {
      const data = await providersService.getVisitDetails(linkedVisitId);

      // Adaptation layer if response structure differs
      const visit = data.data || data;

      setVisitDetails(visit);
      setLinkedMemberId(visit.memberId);
      setLinkedMemberName(visit.memberName);
      setLinkedVisitDate(visit.visitDate);
      setLinkedVisitType(visit.visitType);
      setLinkedProviderName(visit.providerName);
      setLinkedEmployerName(visit.employerName);
      setLinkedMemberCivilId(visit.memberCivilId);
      setLinkedMemberCardNumber(visit.memberCardNumber); // Assuming this field exists

      // If member ID is found, fetch their limit
      if (visit.memberId) {
        fetchMemberLimit(visit.memberId);
      }
    } catch (err) {
  ...
  const fetchAvailableServices = async () => {
    ...
    try {
      const data = await providersService.getContractServices({ size: 2000 });
      ...
      try {
        const services = await providersService.getProviderServices();
        setAvailableServices(
          services.map((s) => ({
            id: s.serviceId || s.id,
  ...
  const fetchMemberLimit = async () => {
    ...
    try {
      const data = await providersService.getMemberRemainingLimit(linkedMemberId);
      setMemberLimit(data);
    } catch (err) {
  ...
  const fetchContractPrice = useCallback(
    async (serviceCode, lineId) => {
      ...
      try {
        const response = await providersService.getServicePrice(serviceCode);
        ...
        setClaimLines((prev) =>
          prev.map((line) => {
            if (line.id === lineId) {
              return {
                ...line,
                unitPrice: priceData.contractPrice || 0,
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      // 1. Prepare Claim JSON Object (Canonical ProviderClaimRequest)
      const claimData = {
        memberId: parseInt(linkedMemberId),
        visitId: parseInt(linkedVisitId),
        claimType: 'DIRECT_BILLING',
        serviceType: linkedVisitType || 'OUTPATIENT',
        serviceDate: linkedVisitDate || new Date().toISOString().split('T')[0],
        claimedAmount: totalClaimAmount,
        diagnosisCode: formData.diagnosisCode,
        diagnosisDescription: formData.diagnosisDescription,
        notes: formData.notes,
        preAuthorizationId: formData.preAuthorizationId ? parseInt(formData.preAuthorizationId) : null,
      ...
      // 2. Decide: With Attachments (Atomic) or JSON Only
      if (pendingFiles.length > 0) {
        // Prepare FormData for Atomic Upload
        const formDataPayload = new FormData();
        formDataPayload.append('claim', JSON.stringify(claimData));

        pendingFiles.forEach((fileObj) => {
          formDataPayload.append('attachments', fileObj.file);
        });

        // Use Atomic Endpoint
        result = await providersService.submitClaimWithAttachments(formDataPayload);
      } else {
        // JSON Only
        result = await providersService.submitClaim(claimData);
      }
      ...
      setSuccess({
        ...
        referenceNumber: result.claimReferenceNumber,
      ...
      const errorMsg = err.message || 'فشل في تقديم المطالبة';
      setError(errorMsg);
    } finally {
      setSubmitting(false);
    }

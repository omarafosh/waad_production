...
export default function ProviderEligibilityCheck() {
  ...
  const checkEligibility = useCallback(async (query) => {
    ...
    try {
      ...
      const response = await providersService.checkEligibility({
        barcode: cleanQuery,
  ...
  useEffect(() => {
    ...
    const fetchEmployers = async () => {
      ...
      try {
        const res = await providersService.getAllowedEmployers();
                        ...
                        <Stack spacing={2}>
                          ...
                          <Button
                            ...
                            onClick={async () => {
                              ...
                              setVisitError(null);
                              try {
                                const visit = await providersService.registerVisit({
                                ...
                                console.error(err);
                                // Use the parsed error message if available, otherwise fallback
                                const msg = err.parsedError?.message || err.message || 'فشل في تسجيل الزيارة';
                                setVisitError(msg);
                              } finally {

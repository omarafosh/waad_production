...
const ProviderPreApprovalSubmission = () => {
  ...
  useEffect(() => {
    const loadContractData = async () => {
      ...
      try {
        ...
        const contractData = await providersService.getProviderContract();
        if (contractData) {
          setContract(contractData);
        }

        const categoriesData = await providersService.getMedicalCategories();
  ...
  useEffect(() => {
    const loadServices = async () => {
      ...
      try {
        ...
        // Use the endpoint that checks member's benefit policy for PA requirements
        const servicesData = await providersService.getServicesRequiringPreAuth({
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const response = await providersService.submitPreApproval(payload);

      // response from service is already unwrapped (data object)
      // Usually it returns the ID or the created object
      const newId = response.id || response.data?.id;

      if (newId) {
        setCreatedPreApprovalId(newId);

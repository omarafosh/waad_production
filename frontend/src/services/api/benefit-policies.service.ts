...
export const getEffectiveBenefitPolicy = async (employerOrgId, date = null) => {
  const params: { employerOrgId: any; date?: string } = { employerOrgId };
...
/**
 * Clone a benefit policy
 * Endpoint: POST /api/benefit-policies/{id}/clone
 * @param {number} id - Source policy ID
 * @param {Object} payload - Clone options (name, targetEmployerId, etc.)
 * @returns {Promise<Object>} Cloned policy
 */
export const cloneBenefitPolicy = async (id, payload = {}) => {
  const response = await axiosClient.post(`${BASE_URL}/${id}/clone`, payload);
  return unwrap(response);
};
 ...
 * @param {Object} payload - Simulation request (policyId, serviceId, encounterType)

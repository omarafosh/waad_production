...
/**
 * Get only DELETED rules for a policy (paginated)
 * Endpoint: GET /api/benefit-policies/{policyId}/rules/deleted
 * @param {number} policyId - Policy ID
 * @param {Object} params - Pagination parameters
 * @returns {Promise<Object>} Paginated response
 */
export const getDeletedPolicyRulesPaged = async (policyId, params = {}) => {
  const response = await axiosClient.get(`/benefit-policies/${policyId}/rules/deleted`, { params });
  return unwrap(response);
};

...
/**
 * Reclassify a service with smart coverage strategy
 * @param {number} id - Service ID
 * @param {Object} payload - ReclassifyRequestDto
 * @returns {Promise<Object>} Updated service
 */
export const reclassifyService = async (id, payload) => {
  const response = await axiosClient.post(`${BASE_URL}/${id}/reclassify`, payload);
  return unwrap(response);
};
 ...
 */
export const getReclassifyImpact = async (id, newCategoryId) => {
  const response = await axiosClient.get(`${BASE_URL}/${id}/reclassify/impact`, {

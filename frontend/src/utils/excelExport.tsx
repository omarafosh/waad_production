...
const getCompanySettings = async (): Promise<CompanySettings> => {
  ...
  try {
    ...
    cacheTimestamp = now;
    return cachedCompanySettings as CompanySettings;
  } catch (error) {
...
export interface ExcelColumn {
  ...
  header: string;
  width?: number;
  type?: 'number' | 'currency' | 'boolean' | 'string';
  format?: string;
 ...
 * 
 * @param {ExportExcelOptions} options Export options
 */
export const exportToExcel = async ({
  ...
  data,
  sheetName = 'البيانات'
}: ExportExcelOptions): Promise<boolean> => {
 ...
 */
export const exportMedicalServicesToExcel = async (data: any[]): Promise<boolean> => {
 ...
 */
export const exportMedicalCategoriesToExcel = async (data: any[]): Promise<boolean> => {
  const columns: ExcelColumn[] = [
    { key: 'code', header: 'الرمز', width: 15, align: 'center' },
    { key: 'name', header: 'الاسم', width: 35, align: 'right' },

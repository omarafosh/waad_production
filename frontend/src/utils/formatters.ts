...
export const LOCALE: string = getNumberLocale(); // Changed to dynamic call
export const CURRENCY_CODE: string = 'LYD';
export const CURRENCY_SYMBOL: string = 'د.ل';
 ...
 */
export const formatCurrency = (value: number | string | null | undefined, showSymbol: boolean = true): string => {
  if (value === null || value === undefined) return '-';
  const num = typeof value === 'string' ? parseFloat(value) : value;
  if (isNaN(num)) return '-';
 ...
 * @param {number | string | null | undefined} value - Percentage value
 ...
 * @param {number | string | null | undefined} value - Amount to format
 */
export const formatCurrencyLYD = (value: number | string | null | undefined): string => formatCurrency(value);
 ...
 * @param {string | Date | null | undefined} date - Date to extract time from

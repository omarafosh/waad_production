 ...
 * safeArray(null) // []
 */
export const safeArray = <T = any>(response: any, preferredKey: string | null = null): T[] => {
 ...
 */
export const safePagination = (response: any, defaults: Partial<PaginationMetadata> = {}): PaginationMetadata => {
  const defaultValues: PaginationMetadata = {
    page: 1,
    size: 20,
    total: 0,
    totalPages: 0,
 ...
 */
export const safeMap = <T = any, U = any>(data: any, mapper: (item: T, index: number, array: T[]) => U): U[] => {
  const arr = safeArray<T>(data);
  if (typeof mapper !== 'function') return arr as any;
 ...
 */
export const hasItems = (data: any): boolean => {
 ...
 */
export const safeRender = (data: any, renderContent: (items: any[]) => any, emptyState: any = null): any => {
 ...
 */
export const safeSelectOptions = (response: any, config: SelectOptionConfig = {}): SelectOption[] => {

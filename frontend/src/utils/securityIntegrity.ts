...
import { SystemRole, RolePrivilegeLevel, SUPER_ADMIN_ONLY_DOMAINS, getPrivilegeLevel } from 'constants/rbac';
...
export const SECURITY_RULES: Record<string, SecurityRule> = {
  ...
  SUPER_ADMIN_CANNOT_BE_DELETED_BY_OTHERS: {
    ...
    verify: (currentRole: string, targetRole: string, action: string) => {
  ...
  SUPER_ADMIN_CANNOT_BE_MODIFIED_BY_OTHERS: {
    ...
    verify: (currentRole: string, targetRole: string, action: string) => {
      if (['UPDATE', 'MODIFY', 'CHANGE_ROLE'].includes(action) && 
          targetRole === SystemRole.SUPER_ADMIN) {
        return currentRole === SystemRole.SUPER_ADMIN;
      }
      return true;
    }
  ...
  SUPER_ADMIN_ROLE_CANNOT_BE_ASSIGNED_BY_OTHERS: {
    ...
    verify: (currentRole: string, roleToAssign: string) => {
  ...
  MODIFY_LOWER_ONLY: {
    ...
    verify: (currentRole: string, targetRole: string) => {
  ...
  RBAC_DOMAIN_SUPER_ADMIN_ONLY: {
    ...
    verify: (currentRole: string, domain: string) => {
  ...
  // INSURANCE_ADMIN Boundaries
  INSURANCE_ADMIN_NO_RBAC_ACCESS: {
    id: 'IA-001',
    description: 'INSURANCE_ADMIN has no RBAC management access',
    verify: (currentRole: string, domain: string) => {
      if (currentRole === SystemRole.INSURANCE_ADMIN && 
          SUPER_ADMIN_ONLY_DOMAINS.includes(domain)) {
        return false;
      }
      return true;
    }
 ...
 * @returns {RuleVerificationResult}
 */
export const verifyRule = (ruleId: string, ...args: any[]): RuleVerificationResult => {
...
export interface AllRulesVerificationResult {
  allPassed: boolean;
 ...
 * @param {string | null} domain - Domain being accessed (if applicable)
 * @returns {AllRulesVerificationResult}
 */
export const verifyAllRules = (currentRole: string, targetRole: string, action: string, domain: string | null = null): AllRulesVerificationResult => {
  ...
  if (!allPassed && import.meta.env.DEV) {
    console.error('🚨 Security Rules Verification Failed:', 
...
export interface IntegrityChecksSummary {
  allPassed: boolean;
 ...
 */
export const runIntegrityChecks = (): IntegrityChecksSummary => {
  ...
  const allRolesHaveLevels = Object.values(SystemRole)
    .every(role => RolePrivilegeLevel[role as string] !== undefined);
  checks.push({
  ...
  if (!allPassed) {
    console.error('🚨 SECURITY INTEGRITY CHECK FAILED:', checks.filter(c => !c.passed));

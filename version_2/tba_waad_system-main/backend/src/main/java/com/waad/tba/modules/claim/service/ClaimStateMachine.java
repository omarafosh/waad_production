...
public class ClaimStateMachine {
     ...
     * 
     * NOTE (2026-02-01): APPROVED → SETTLED transition is DISABLED.
     * All settlements must go through Settlement Batches.
     * The transition is still technically valid for system-level operations
     * (e.g., when batch is paid), but not exposed to users.
     */
    private Set<String> getRequiredRoles(ClaimStatus from, ClaimStatus to) {
        return switch (from) {
            ...
            case SUBMITTED -> switch (to) {
                case SUBMITTED -> Set.of(ROLE_ACCOUNTANT, ROLE_REVIEWER);
                default -> Set.of();
            };
            ...
            // APPROVED → SETTLED is ONLY allowed via SettlementBatchService (system-level)
            // No user role can directly transition a claim to SETTLED anymore
            case APPROVED -> Set.of(); // Empty = no direct user transition allowed
            default -> Set.of();
        };
    }

...
public class ProviderClaimsService {
     ...
     * 
     * FIXED (Audit Fix): Now uses BenefitPolicyCoverageService for real annual limit tracking.
     */
    private AnnualLimitCheck checkAnnualLimit(Member member, BigDecimal claimedAmount) {
        ...
        if (annualLimit == null || annualLimit.compareTo(BigDecimal.ZERO) <= 0) {
            // Unlimited policy
            return new AnnualLimitCheck(BigDecimal.ZERO, BigDecimal.ZERO, claimedAmount, BigDecimal.ZERO, BigDecimal.ZERO, 0, 0, false, new ArrayList<>());
        }

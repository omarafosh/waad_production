...
const KPICard = ({ title, value, subtitle, icon: Icon, color = 'primary', loading = false, trend, onClick, warning = false }) => {
...
const StatusBadge = ({ status }) => {
  const statusConfig = {
    ...
    APPROVAL_IN_PROGRESS: { label: 'جاري معالجة الموافقة', color: 'warning', icon: PendingIcon },
    APPROVED: { label: 'معتمدة', color: 'primary', icon: CheckCircleIcon },
    BATCHED: { label: 'ضمن دفعة تسوية', color: 'secondary', icon: PendingIcon },
    REJECTED: { label: 'مرفوضة', color: 'error', icon: CancelIcon },
    SETTLED: { label: 'مسددة', color: 'success', icon: CheckCircleIcon },
    NEEDS_CORRECTION: { label: 'تحتاج تصحيح', color: 'warning', icon: WarningAmberIcon }
  };
...
export default function Dashboard() {
  ...
  // All authenticated users can view claims — backend enforces data scoping
  const canViewClaims = true;

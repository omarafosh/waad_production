...
const ProviderContractView = () => {
  ...
  const handleExcelUpload = useCallback(
    async (file) => {
      try {
        ...
        if (result.success) {
          enqueueSnackbar(result.message || `تم استيراد ${result.summary?.inserted + result.summary?.updated || 0} بند تسعير بنجاح`, {
            variant: 'success'
          });
  ...
  const handleImportPriceList = useCallback(async () => {
    try {
    ...
    <>
                    ...
                    <TableRow key={item.id || index} hover sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                      ...
                      <TableCell>
                        <Typography variant="body2">
                          {item.serviceName || item.medicalService?.name || item.service?.name || '-'}
                        </Typography>
                      </TableCell>
                      <TableCell>
                        <Typography variant="body2" color="text.secondary">
                          {item.effectiveCategory?.name || item.medicalCategory?.name || item.categoryName || item.category?.name || '-'}
                        </Typography>
                      </TableCell>
        ...
        <DialogContent>
          ...
          <Stack spacing={3}>
            ...
            <Autocomplete
              ...
              renderInput={(params) => (
                <TextField {...params} label="التصنيف الطبي (اختياري)" helperText="اختر تصنيفاً مختلفاً عن التصنيف الافتراضي للخدمة" />
              )}
            />
      ...
      <Dialog open={editPricingDialogOpen} onClose={() => setEditPricingDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <DialogContentText sx={{ mb: 3 }}>
            {selectedPricingItem
              ? `تعديل السعر للخدمة: ${selectedPricingItem.serviceName || selectedPricingItem.medicalService?.name}`
              : 'تعديل السعر'}
          </DialogContentText>
          <Stack spacing={3}>
            <TextField
            ...
            <Autocomplete
              ...
              renderInput={(params) => (
                <TextField {...params} label="التصنيف الطبي (اختياري)" helperText="يمكنك تغيير التصنيف لهذه الخدمة" />
              )}
            />

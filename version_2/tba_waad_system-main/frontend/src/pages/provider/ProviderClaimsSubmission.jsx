...
export default function ProviderClaimsSubmission() {
  ...
  const fetchAvailablePreAuths = async () => {
    ...
    try {
      ...
      const preAuths = response.data?.data || response.data || [];
      ...
      <Stack spacing={3}>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={6}>
            <InfoCard bgcolor={(theme) => alpha(theme.palette.success.main, 0.04)}>
              ...
              <Grid container spacing={2}>
                <Grid item xs={12}>
                  <ReadOnlyField icon={PersonIcon} label="اسم المؤمن عليه" value={linkedMemberName} highlight />
                </Grid>
                <Grid item xs={6}>
        ...
        <FormSection highlighted>
          ...
          <Alert severity="info" icon={<InfoIcon />} sx={{ mb: 2, borderRadius: 2 }}>
            <Typography variant="body2">📋 جميع الخدمات الطبية معروضة - الخدمات التي تتطلب موافقة مسبقة تظهر مع علامة 🟡</Typography>
          </Alert>
        ...
        {/* ═══════════════════════ ROW 3: PRE-AUTH + DIAGNOSIS ═══════════════════════ */}
        <Grid container spacing={3}>
          {/* Pre-Authorization Selection (PHASE 5) */}
          <Grid item xs={12} md={6}>
            <FormSection>
              <SectionHeader icon={HealingIcon} title={LABELS.preAuth} subtitle="ربط المطالبة بموافقة مسبقة" color="success" />
              <Divider sx={{ mb: 3 }} />
